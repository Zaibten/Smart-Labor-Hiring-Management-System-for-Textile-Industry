...
const checkIfApplied = async (jobId: string) => {
  try {
    const res = await fetch(`https://labour-server.vercel.app/api/check-application/${jobId}?email=${user.email}`);
...
const fetchUserImage = async (email: string) => {
  ...
  try {
    const res = await fetch(`https://labour-server.vercel.app/api/user-by-email/${email}`);
    if (!res.ok) {
...
useEffect(() => {
  const fetchUserAndJobs = async () => {
    try {
      ...
      // fetch skills
      const skillsRes = await fetch(`https://labour-server.vercel.app/api/user/skills/${parsedUser.email}`);
      ...
      if (parsedUser.role === "Contractor") {
        const resMine = await fetch(`https://labour-server.vercel.app/api/my-jobs-email/${parsedUser.email}`);
  ...
  try {
    const res = await fetch(
      `https://labour-server.vercel.app/api/search-jobs?name=${text}&skill=${text}`
    );
...
const handleApply = async (job: Job) => {
  try {
    const response = await fetch(`https://labour-server.vercel.app/api/jobs/apply/${job._id}`, {
      method: "POST",
