...
useEffect(() => {
  ...
  const fetchSkills = async (email: string) => {
    try {
      const res = await fetch(`https://labour-server.vercel.app/api/user/skills/${email}`);
  ...
  const addSkill = async () => {
    ...
    try {
      await fetch(`https://labour-server.vercel.app/api/user/${userEmail}/skills`, {
  method: "POST",
    ...
    Alert.alert("Remove Skill", "Are you sure you want to delete this skill?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await fetch(
  `https://labour-server.vercel.app/api/user/${userEmail}/skills/${index}`,
  { method: "DELETE" }
);
